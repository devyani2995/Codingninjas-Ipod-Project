...
// Stateful App Class Component to Render the App
class App extends React.Component {
	...
	constructor() {
		...
		this.state = {
			...
			// State for managing the Mouse Click CSS Effect
			mouse: {
				innerCircle: "",
			},
	...
	// Invoked when the page is loaded and uses the Reference to the Controller
	componentDidMount() {
	...
	// Invoked when we press the Play/Pause Button to Play-Pause the Song
	play = (songsList) => {
		if (
	...
	// Invoked when we Press the Next Button for the Next Song
	nextSong = (songsList) => {
		if (
			this.state.menu.pageRender === "yes" &&
			this.state.screen.screenIndex === 7
	...
	// Invoked when we Press the Previous Button for the Previous Song
	prevSong = (songsList) => {
		if (
			this.state.menu.pageRender === "yes" &&
			this.state.screen.screenIndex === 7
	...
	// To renders the App Component
	render() {
		const { menu, screen, mouse, songsList, theme } = this.state;
		
		// for changing the Application Body Theme
		const styling = () => {
			if (theme.themeIndex === 0) {
